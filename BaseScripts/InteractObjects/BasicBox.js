...
import {DeltaTime, cameraX, cameraY, cameraIntX, cameraIntY, drawImage, Inputs, lerp, LevelX, DEG2RAD, RAD2DEG, clamp,MoveCamTarget, magnitude,MasterArrayLevelSize,MissItems} from "../../index.js";
...
export default class BasicBox{
    ...
    Draw(){
        drawImage(ctx,MissItems,0,0, this.SpriteWidth, this.SpriteHeight, (Math.round(this.position.x - cameraIntX - (this.SpriteWidth / 2) * this.SpriteScale)),Math.round((this.position.y - cameraIntY - (this.SpriteHeight / 2) * this.SpriteScale)), this.SpriteWidth * this.SpriteScale, this.SpriteHeight * this.SpriteScale,this.angle);
    }
    CollisionDect(){
    ...
    Update(){
        ...
        this.velocity.y = lerp(this.velocity.y, 0, 2 * DeltaTime);
    ...
    Damage(amount, x, y){
        const diffX = x - this.position.x;
        const diffY = y - this.position.y;
    }
    Death(){
